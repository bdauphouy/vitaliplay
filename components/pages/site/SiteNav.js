...
const SiteNav = () => {
  const { sitePages, otherPages, externalPages, getPage, SITE_PREFIX } =
  ...
  useEffect(() => {
    ...
    navItems.forEach((navItem) => {
      ...
      console.log(getPage(sitePages, 'pageName', navItem.innerText).path)

      if (
        router.asPath === getPage(sitePages, 'pageName', navItem.innerText).path
      ) {
        updateMarker()
      }
    })
                    ...
                    className={`nav-item inline-flex h-full items-center px-6 font-head text-lg font-semibold ${
                      sitePage.pageName === 'Accueil'
                        ? router.asPath ===
                          ...
                          ? 'text-blue-900'
                          : 'text-dark-300'
                        : router.asPath.includes(sitePage.path)
                        ? 'text-blue-900'
                        : 'text-dark-300'
                    }`}
                  >
